...
Step 1 (authorize):
- Connects to google_drive: http://localhost:3000/google_drive/authorize
- This will get redirected to configured callback url after successful authorization (AppConfig.redirect_uri) ie: http://localhost:3000/google_drive/callback
...
- This will trigger fetch content job to pull google_drive contents for each source.
...
http://localhost:3000/google_drive/authorize?organization_id=&source_type_id=&client_host=localhost
...
```
  ...
  require 'openssl'
  require 'json'
  key_hash = {organization_id: 439, client_host: 'http://es.lvh.me:4000', source_type_id:'3bd195bc-8526-479f-a8a2-08a1a087868c'}.to_json
  ...
  puts "http://localhost:3000/google_drive/authorize?auth_data=#{encode_key}&secret=#{digest_key}"
```
 # To run webhook using rails console
  - Input: For `webhook_type=source`, pass `source_type_id` along with `source_id` as well.
```
input = {:webhook_type=>"source", :id=>"883fa90b-59fa-4e57-99b0-09a59ad2aa7e", :source_type_id=>"3bd195bc-8526-479f-a8a2-08a1a087868c"}
