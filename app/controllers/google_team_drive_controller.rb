class GoogleTeamDriveController < ApplicationController
  ...
  def authorize
    ...
    auth_url = client.auth_code.authorize_url(
      redirect_uri: callback_google_team_drive_index_url,
      scope: [Google::Apis::DriveV3::AUTH_DRIVE_READONLY, 'https://www.googleapis.com/auth/userinfo.email'].join(' '),
      access_type: 'offline',
  ...
  def callback
    if params[:code]
      get_token_from_code(params[:code])
  ...
  def get_team_drive_content(drive_sesion)
    ...
    paginated_files = []
    loop do
      (files, page_token) = drive_sesion.files(
        q: "#{query} in parents",
