class OneDriveController < ApplicationController
  ...
  def callback
    begin
      ...
      if @access_token && @email && @name
        ...
        if provider
          redirect_to fetch_folders_one_drive_index_path(
      ...
      begin
        ...
        if @unauthorized_parameters
          ...
          sharepoint_communicator = SharepointCommunicator.new(
            client_id:     AppConfig.integrations['one_drive']['client_id'],
            client_secret: AppConfig.integrations['one_drive']['client_secret'],
            token:         record.token
          )
          @folders = sharepoint_communicator.folders("/v1.0/me/drive/root/children")["value"]
          @folders = @folders.select {|folder| folder["folder"]}
        end
      rescue Exception => he
        redirect_to authorize_one_drive_index_path
      end
    else
  ...
  def source_params
    params.permit(:state, :provider_id, folders: {})
  end
